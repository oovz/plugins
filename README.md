...
|---|---:|---|
| [Senior Engineering Workflow](plugins/senior-engineering-workflow/) | 0.9.3 | Keeps coding decisions in the capable main agent while bounded Researcher, Engineer, Verifier, and Worker roles isolate implementation and noisy evidence when useful. |
...
By default the four roles run on your main agent's model, with its thinking level, tools, and permissions. To route some roles to another model, create model-only aliases with the same CLI:
...
Three-model routing is available through `--preset three-model`. Use `--preset inherit` to remove only aliases generated by `@oovz/sew` and return to full inheritance. Antigravity is the exception: its custom agents require an explicit tool list, so no aliases are generated there.
