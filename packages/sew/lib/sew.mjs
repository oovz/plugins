...
Install/update/uninstall options:
  ...
  --force                        Replace conflicting static files or force a supported native install
  --json                         Emit JSON
...
function staticInstallRoots(host, scope, project, env = process.env) {
  ...
  if (host === "codex") {
    ...
    return {
      ...
      skills: scope === "project" ? path.join(project, ".agents", "skills") : path.join(home, ".agents", "skills"),
    };
  }
...
async function staticPlan(host, scope, project, env = process.env) {
  ...
  if (host === "codex") {
    for (const artifact of artifacts) {
      if (artifact.path.startsWith("companion/agents/")) files.push({ root: "agents", path: normalizeRelative(artifact.path.slice("companion/agents/".length)), content: artifact.content });
      if (artifact.path.startsWith("skills/")) files.push({ root: "skills", path: normalizeRelative(artifact.path.slice("skills/".length)), content: artifact.content });
    }
...
async function planStaticOperation(operation, host, scope, project, options, env = process.env) {
  ...
  if (operation === "install" && currentState) throw new CliError(`Senior Engineering Workflow is already installed for ${host}/${scope}; use sew update.`, 1);
...
function executeNative(commands, options = {}) {
  ...
  for (const command of commands) {
    ...
    const result = runner(executable, args, { cwd: command.cwd, encoding: "utf8", stdio: options.json ? "pipe" : "inherit", shell: false });
    const stderr = options.json ? String(result.stderr ?? "") : "";
    const stdout = options.json ? String(result.stdout ?? "") : "";
...
async function runInstallOperation(operation, options, runtime = {}) {
  ...
  const actions = operation === "uninstall"
